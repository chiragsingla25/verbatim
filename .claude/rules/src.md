...
- **Retrieval only ever goes through the `security invoker` RPC** — `match_chunks_hybrid` on
  the `/ask` path (dense pgvector `<=>` + lexical `tsvector`, fused by RRF k=60), or the
  retained pgvector-only `match_chunks` — called with the end user's Supabase client / JWT.
  Both RRF arms filter `version_id`. Never query `document_chunks` from an Edge Function with
  the service-role client to answer a user question — that bypasses RLS and the version
  boundary. A metadata question may also be grounded in the deterministic `__facts__` chunk
  from `version_facts()` (also `security invoker`); it is never a retriever result and is
  excluded from `query_log.retrieved`.
- **The vector store is Postgres.** `document_chunks.embedding` is `vector(384)`; the lexical
  arm is a generated `document_chunks.tsv tsvector` + GIN index — same database, no external
  vector or search service. Dense similarity is pgvector `<=>` (HNSW); access control is the
  table's RLS policy, enforced identically on both arms.
...
- **No reranker.** Retrieve `k≈10–12` (hybrid: dense + lexical, fused by RRF — rank fusion is
  not a reranker, no model) and pass straight to the answer step. If a real reranker is added
  later it runs client-side or in its own service — never assume PyTorch in an Edge Function.
